...
public class Chain {
    ...
    public boolean add(Block NewBlock) {
        ...
        Tree PrevNode = find(Root, NewBlock.PrevNodeNumber);
    ...
    private Tree find(Tree root, int PrevNodeNumber) {
        if (root == null) {
            return null;
        }
    ...
    private boolean exists(Tree root, Block NBlock) {
        ...
        if (root.block.PrevNodeNumber == NBlock.PrevNodeNumber && root.block.TimeStamp == NBlock.TimeStamp && root.block.MinerId==NBlock.MinerId) {
            return true;
        }
        for (int i = 0; i < root.Childs.size(); i++) {
            Block temp = root.Childs.get(i).block;
